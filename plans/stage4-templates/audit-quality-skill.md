...
user-invocable: true
argument-hint: "[symptom hint, e.g. \"researcher feels off on Hey Lexxi tasks since last week\"]"
context: fork
...
```
/audit-quality
/audit-quality "researcher feels off on Hey Lexxi tasks since last week"
...
1. **Pin the agent context.** The auditor's instructions live in `.claude/agents/quality-auditor.md`. The shared rubric is `vault/shared/regression-replay/rubric.md` (currently v2). Both should be read at the start of every invocation so the auditor's verdicts remain consistent across sessions.
...
Today only 1 of 10 seeds is pinned (shape-01 = "GraphN reliability"). Most regressions won't surface in the timeline yet. The auditor should be honest about coverage limits and suggest pinning more shapes if the user's concern falls outside the current pin's domain.
